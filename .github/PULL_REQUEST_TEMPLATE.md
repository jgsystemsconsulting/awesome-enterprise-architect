...
- [ ] On-topic for Sparx Systems Enterprise Architect product use (not general TOGAF/ArchiMate-only EA)
...
- [ ] Not a duplicate (canonical-URL rule, CONTRIBUTING.md §6; not a copy of an ArchiMate spoke entry)
...
- [ ] Tool tags always include `Sparx-EA`
- [ ] `(YYYY)` is the resource's most recent author-published version
- [ ] If `has-model`: directly downloadable, non-paywalled `.eap` / `.eapx` / `.qea` (CONTRIBUTING.md §3)

## Housekeeping

- [ ] If a **top-level** section was added/renamed, updated the hand-maintained `## Contents` ToC
- [ ] (If a JGS product) it sits next to ≥1 competing/alternative entry (CONTRIBUTING.md Editorial neutrality)
